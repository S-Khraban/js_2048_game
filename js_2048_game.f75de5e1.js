'use strict';
/* global Game */ const $cells = Array.from(document.querySelectorAll('.field-row .field-cell'));
...
const game = new Game(typeof window !== 'undefined' ? window.initialState : undefined);
...
function renderBoard(state) {
    let i = 0;
    for (const row of state)for (const value of row){
        const cell = $cells[i++];
        cell.textContent = value ? String(value) : '';
        Array.from(cell.classList).filter((cls)=>cls.startsWith('field-cell--') || cls === 'spawn' || cls === 'bump').forEach((cls)=>cell.classList.remove(cls));
...
function animateChanges(prev, next) {
    ...
    for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
        ...
        if (was === 0 && now > 0) {
            ...
            cell.addEventListener('animationend', ()=>cell.classList.remove('spawn'), {
                once: true
            });
...
function compressRowWithMappingLeft(row, r) {
    const src = [];
    for(let c = 0; c < 4; c++)if (row[c] !== 0) src.push({
        val: row[c],
        c
    });
    const moves = [];
    let dest = 0;
    for(let i = 0; i < src.length; i++)if (i + 1 < src.length && src[i].val === src[i + 1].val) {
        moves.push({
            fromR: r,
            fromC: src[i].c,
            toR: r,
...
function planMoves(prev, dir) {
    const moves = [];
    if (dir === 'left') for(let r = 0; r < 4; r++)moves.push(...compressRowWithMappingLeft(prev[r], r));
    else if (dir === 'right') for(let r = 0; r < 4; r++){
        const rev = [
            ...prev[r]
        ].reverse();
    ...
    else if (dir === 'up') for(let c = 0; c < 4; c++){
        const col = [
            prev[0][c],
            prev[1][c],
            prev[2][c],
    ...
    else if (dir === 'down') for(let c = 0; c < 4; c++){
        const col = [
            prev[3][c],
            prev[2][c],
            prev[1][c],
...
function playSlideAnimation(moves, done) {
    ...
    for(let i = 0; i < ghosts.length; i++)ghosts[i].addEventListener('transitionend', onEnd, {
        once: true
    });
    setTimeout(()=>{
...
//# sourceMappingURL=js_2048_game.f75de5e1.js.map
