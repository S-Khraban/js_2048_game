...
class Game {
    constructor(initialState){
        this.size = 4;
        this._initial = Array.isArray(initialState) && initialState.length === this.size ? initialState.map((row)=>row.slice()) : this._emptyBoard();
    ...
    _spawnRandomTile() {
        ...
        for(let r1 = 0; r1 < this.size; r1++){
            for(let c1 = 0; c1 < this.size; c1++)if (this._board[r1][c1] === 0) empties.push([
                r1,
                c1
            ]);
        }
    ...
    _reverseRows() {
        for(let r1 = 0; r1 < this.size; r1++)this._board[r1].reverse();
    }
    _transpose() {
        const n = this.size;
        for(let r1 = 0; r1 < n; r1++)for(let c1 = r1 + 1; c1 < n; c1++){
    ...
    _moveLeftInternal() {
        ...
        for(let r1 = 0; r1 < this.size; r1++){
            ...
            const merged = [];
            for(let i = 0; i < compressed.length; i++)if (compressed[i] === compressed[i + 1]) {
            ...
            for(let c1 = 0; c1 < this.size; c1++){
                if (this._board[r1][c1] !== merged[c1]) anyChange = true;
    ...
    _canMove() {
        if (this._board.some((r1)=>r1.some((v)=>v === 0))) return true;
        const n = this.size;
...
//# sourceMappingURL=js_2048_game.4bb9c666.js.map
